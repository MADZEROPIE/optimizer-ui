...
void GKLSFunction::SetFunctionNumber(int number) {
...
int GKLSFunction::GKLS_parameters_check() const {
    ...
    for (i = 1; i < GKLS_dim; i++)
        if ((tmp = GKLS_domain_right[i] - GKLS_domain_left[i]) < min_side)
            min_side = tmp;
    if ((GKLS_global_dist >= 0.5 * min_side - GKLS_PRECISION) || (GKLS_global_dist <= GKLS_PRECISION))
        return GKLS_GLOBAL_DIST_ERROR; /* global distance error */
    if ((GKLS_global_radius >= 0.5 * GKLS_global_dist + GKLS_PRECISION) || (GKLS_global_radius <= GKLS_PRECISION))
...
int GKLSFunction::GKLS_arg_generate(unsigned int nf) {
    ...
    for (i = 0; i < GKLS_dim; i++) {
        GKLS_minima.local_min[0][i] =
            GKLS_domain_left[i] + rnd_num[rnd_counter] * (GKLS_domain_right[i] - GKLS_domain_left[i]);
        rnd_counter++;
    ...
    mRndGenerator.GenerateNextNumbers();  // ranf_array(rnd_num, NUM_RND);
    rnd_counter = 0L;
    GKLS_minima.local_min[1][0] = GKLS_minima.local_min[0][0] + GKLS_global_dist * cos(PI * rnd_num[rnd_counter]);
    ...
    for (j = 1; j < GKLS_dim - 1; j++) {
        GKLS_minima.local_min[1][j] =
            GKLS_minima.local_min[0][j] + GKLS_global_dist * cos(2.0 * PI * rnd_num[rnd_counter]) * sin_phi;
    ...
    GKLS_minima.local_min[1][GKLS_dim - 1] = GKLS_minima.local_min[0][GKLS_dim - 1] + GKLS_global_dist * sin_phi;
        ...
        while (i < GKLS_num_minima) {
            do {
                ...
                for (j = 0; j < GKLS_dim; j++) {
                    GKLS_minima.local_min[i][j] =
                        GKLS_domain_left[j] + rnd_num[rnd_counter] * (GKLS_domain_right[j] - GKLS_domain_left[j]);
                    rnd_counter++;
                    if (rnd_counter == NUM_RND) {
                        mRndGenerator.GenerateNextNumbers();  // ranf_array(rnd_num, NUM_RND);
                        rnd_counter = 0L;
            ...
            } while ((GKLS_global_radius + gap) - GKLS_norm(GKLS_minima.local_min[i], GKLS_minima.local_min[1]) >
                     GKLS_PRECISION);
            i++;
        }
        error = GKLS_coincidence_check();
    } while ((error == GKLS_PARABOLA_MIN_COINCIDENCE_ERROR) || (error == GKLS_LOCAL_MIN_COINCIDENCE_ERROR));
...
int GKLSFunction::GKLS_set_basins() {
    ...
    for (i = 2; i < GKLS_num_minima; i++) {
        if ((dist = (GKLS_norm(GKLS_minima.local_min[i], GKLS_minima.local_min[1]) - GKLS_global_radius -
                     GKLS_PRECISION)) < GKLS_minima.rho[i])
            GKLS_minima.rho[i] = dist;
    }
    ...
    for (i = 0; i < GKLS_num_minima; i++) {
        ...
        {           /* rho(i) := max {rho(i),min[||M(i)-M(j)|| - rho(j): i !=j]},      */
            ...
            for (j = 0; j < GKLS_num_minima; j++)
                if (i != j) {
                    if ((temp_d1 = GKLS_norm(GKLS_minima.local_min[i], GKLS_minima.local_min[j]) - GKLS_minima.rho[j]) <
                        temp_min)
                        temp_min = temp_d1;
                }
...
double GKLSFunction::EvaluateNDFunction(const double* x) const {
    ...
    index = 1;
    while ((index < GKLS_num_minima) && (GKLS_norm(GKLS_minima.local_min[index], x) > GKLS_minima.rho[index]))
        index++;
    ...
    for (i = 0; i < GKLS_dim; i++)
        scal +=
            (x[i] - GKLS_minima.local_min[index][i]) * (GKLS_minima.local_min[0][i] - GKLS_minima.local_min[index][i]);
    /* Return the value of the quadratic interpolation function */
    return ((1.0 - 2.0 / rho * scal / norm + a / rho / rho) * norm * norm + GKLS_minima.f[index]);
}
...
double GKLSFunction::EvaluateD2Function(const double* x) const {
    ...
    return ((-6.0 * scal / norm / rho + 6.0 * a / rho / rho + 1.0 - delta / 2.0) * norm * norm / rho / rho +
...
double GKLSFunction::EvaluateDFunctionDeriv(unsigned var_j, const double* x) const {
    ...
    h = (GKLS_minima.local_min[0][var_j] - GKLS_minima.local_min[index][var_j]) * norm - scal * dif / norm;
    /* Return the value of dC(x)/dx[var_i] of the D-type function */
    return (h * (2.0 / rho / rho * norm - 4.0 / rho) +
            dif * (6.0 / rho / rho * scal - 6.0 / rho / rho / rho * a * norm - 8.0 / rho / norm * scal +
                   6.0 / rho / rho * a + 2.0));
}
...
double GKLSFunction::EvaluateD2FunctionDeriv1(unsigned var_j, const double* x) const {
    ...
    return (h * norm / rho / rho * (-6.0 * norm * norm / rho / rho + 16.0 * norm / rho - 12.0) +
            dif * norm *
                ((-30.0 / rho / norm * scal + 30 / rho / rho * a + 5.0 - 2.5 * delta) / rho / rho / rho * norm * norm +
...
double GKLSFunction::EvaluateD2FunctionDeriv2(unsigned var_j, unsigned var_k, const double* x) const {
    unsigned int i, index;
    double norm, scal, a, rho, dh, difj, difk, hj, hk, dQ_jk; /* working variables */
    ...
    hj = (GKLS_minima.local_min[0][var_j] - GKLS_minima.local_min[index][var_j]) * norm - scal * difj / norm;
    hk = (GKLS_minima.local_min[0][var_k] - GKLS_minima.local_min[index][var_k]) * norm - scal * difk / norm;

    dh =
        (GKLS_minima.local_min[0][var_j] - GKLS_minima.local_min[index][var_j]) * difk / norm - hk * difj / norm / norm;
    if (the_same)
        dh = dh - scal / norm;

    dQ_jk = -6.0 / rho / rho / rho / rho * (dh * norm * norm * norm + 3.0 * hj * difk * norm) -
            30.0 / rho / rho / rho / rho * hk * difj * norm +
            15.0 / rho / rho / rho * (-6.0 / rho * scal / norm + 6.0 / rho / rho * a + 1 - 0.5 * delta) * difj * difk *
                norm +
            16.0 / rho / rho / rho * (dh * norm * norm + 2.0 * hj * difk) + 64.0 / rho / rho / rho * hk * difj +
            8.0 / rho / rho * (16.0 / rho * scal / norm - 15.0 / rho / rho * a - 3.0 + 1.5 * delta) * difj * difk -
            12.0 / rho / rho * (dh * norm + hj * difk / norm) - 36.0 / rho / rho * hk * difj / norm +
            3.0 / rho * (-12.0 / rho * scal / norm + 10.0 / rho / rho * a + 3.0 - 1.5 * delta) * difj * difk / norm;

    if (the_same)
        dQ_jk = dQ_jk +
                5.0 * norm * norm * norm / rho / rho / rho *
                    (-6.0 / rho * scal / norm + 6.0 / rho / rho * a + 1 - 0.5 * delta) +
                4.0 * norm * norm / rho / rho * (16.0 / rho * scal / norm - 15.0 / rho / rho * a - 3.0 + 1.5 * delta) +
                3.0 * norm / rho * (-12.0 / rho * scal / norm + 10.0 / rho / rho * a + 3.0 - 1.5 * delta) + delta;
...
int GKLSFunction::GetGlobalMinimumPoint(int fNumber, double* argmin) const {
    if (fNumber < 1 || fNumber > 100)
        return GKLS_FUNC_NUMBER_ERROR;
    if (isArgSet == 1) {
